...
public class MazeFXApp extends DrawingApp {
		...
		if (currentPos.get() < positions.length) {
			...
			if (currentPos.get() == positions.length - 1) {
				if (x == maze.getTargetX() && y == maze.getTargetY()) {
					...
					log("Não chegou ao ponto alvo. Tente outro genótipo ou labitinto)");
				}
				ctx.fillRect(mazeX, mazeY, rectWidth, rectHeight);
			}
	...
	private void createConfigurationPane() {
		...
		Label lblMazeConfigTitle = new Label("Configurações");
		lblMazeConfigTitle.setFont(fontTitle);

		Label lblLogTitle = new Label("Informações");
		...
		Button btnNewMaze = new Button("Novo Labirinto");
		gpMazeConfig.add(btnNewMaze, 2, 2);

		Button btnNewGenotype = new Button("Novo Genótipo");
		gpMazeConfig.add(btnNewGenotype, 3, 2);

		//Button btnStart = new Button("Evoluir");
		Button btnReplay = new Button("Repetir");

		Button btnStop = new Button("Parar Animação");
		...
		txtLog = new TextArea("Inicializando...");
		...
		HBox n = new HBox(5, new VBox(5, lblMazeConfigTitle, new Separator(), gpMazeConfig),
				...
				//new VBox(5, lblEvolutionTitle, new Separator(), new Label("Número de gerações"), vbGenerations,
