...
public class MazeSolver {
	...
	private Maze maze;
        ...
        static int MaxTam = 60;// maximum number for genes in a genotype
        static int MaxPop = 300;// maximum number for individuos
	...
	public Genotype<AnyGene<Direction>> newGenotype(int MaxTam) {//numberOfGenes
		return Genotype.of(AnyChromosome.of(this::generateRandomDirection, MaxTam));//numberOfGenes));
	}

	public Direction[] evolveToDirections(Genotype<AnyGene<Direction>> genotype, int limit, int MaxPop){//population) {
		Genotype<AnyGene<Direction>> result = evolveToGenotype(genotype, limit, MaxPop);//population);
